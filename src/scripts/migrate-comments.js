...
Request.get("https://gatsby-digett-d8.pantheonsite.io/api/comments.json", (error, response, body) => {
	...
	result.forEach(row => {
		...
		const nodecreated = new Date(row.created_1 * 1000)
		const commentcreated = new Date(row.created * 1000)
		...
		//   file.on('open', function () {
		    file.write('<item>');
				...
				file.write(`<wp:post_date_gmt>${getDisqusDate(nodecreated)}</wp:post_date_gmt>`);
				...
				file.write(`<wp:comment_date_gmt>${getDisqusDate(commentcreated)}</wp:comment_date_gmt>`);
